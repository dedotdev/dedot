...
import { WestendAssetHubApi } from "@dedot/chaintypes"
...
export const run = async (nodeName: any, networkInfo: any) => {
  ...
  const client = await LegacyClient.new<WestendAssetHubApi>(provider);
  ...
  const newClient = await DedotClient.new<WestendAssetHubApi>(provider);
