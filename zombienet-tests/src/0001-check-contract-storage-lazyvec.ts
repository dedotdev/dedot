...
export const run = async (_nodeName: any, networkInfo: any) => {
  ...
  const verifyLazyVecStorage = async (api: ISubstrateClient<SubstrateApi[RpcVersion]>) => {
    ...
    await contract.tx.createProposal(
      new Uint8Array([1, 2, 3, 4, 5]), // data
      100, // duration
      2, // min_approvals
    ...
    assert(proposal!.until === 100, 'Proposal until should be 100');
    ...
    assert(secondProposal!.until === 200, 'Second proposal until should be 200');
