...
import { assert, DedotError, generateRandomHex } from 'dedot/utils';
import { beforeAll, describe, expect, it } from 'vitest';
import { FlipperContractApi } from '../contracts/flipper';
import { FlipperSolContractApi } from '../contracts/flipper-sol';
import { devPairs, flipperV5Metadata, flipperV6Metadata, flipperSolAbi, flipperSolCode } from '../utils.js';
...
describe('ContractDeployer', () => {
  ...
  describe('solidity contract support', () => {
    let deployer: ContractDeployer<FlipperSolContractApi>;

    beforeAll(async () => {
      deployer = new ContractDeployer(reviveClient, flipperSolAbi, flipperSolCode, { defaultCaller: alice.address });
    });
    ...
    it('should deploy sol contract using PVM code properly', async () => {
      ...
      console.log('Deployed sol contract address:', contractAddress);

      const contract = new Contract(reviveClient, flipperSolAbi, contractAddress, {
        defaultCaller: alice.address,
      });

      const { data } = await contract.query.get();
    ...
    it('should handle error properly with assert!', async () => {
      try {
        ...
        expect(e.message).toMatch(/Failed to decode error. Details:/);
      }
    });
