...
import { BASIC_KNOWN_TYPES, NamedType, TypeImports } from '@dedot/codegen';
import { assert, normalizeName, stringPascalCase } from '@dedot/utils';
import { findKnownCodec, findKnownCodecType, isKnownCodecType } from '@dedot/codegen';
import { beautifySourceCode, compileTemplate, isNativeType, WRAPPER_TYPE_REGEX } from '@dedot/codegen';
import path from 'path';
import { extractContractTypes, normalizeContractTypeDef } from '../utils';
...
export class TypeGen {
  ...
  generate(): Promise<string> {
    ...
    const template = compileTemplate('contracts.hbs', path.resolve(process.cwd(), 'packages/contracts/src/gen'));
