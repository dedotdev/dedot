...
import { assert, blake2_256, deferred, hexToU8a, keccak_256, stringCamelCase, stringPascalCase, u8aToHex } from '@dedot/utils';
      ...
      describe('cache disabled', () => {
          ...
          it('should query storage', () => {
            api.metadata.latest.pallets.forEach((pallet) => {
              pallet.storage?.entries.forEach((entry) => {
                ...
                expectTypeOf(
                  api.query[stringCamelCase(pallet.name)][stringCamelCase(entry.name)].rawKey,
                ).toBeFunction();
        ...
        describe('runtime apis call', () => {
          ...
          it('should throws error if runtime not support or call spec not found', async () => {
            expect(() => api.call.metadata.notFound()).toThrowError(
              'Runtime api spec not found for Metadata_not_found',
            );
            expect(() => api.call.notFound.notFound()).toThrowError(
              'Runtime api spec not found for NotFound_not_found',
            );
          });
        });
        ...
        describe('tx calls', () => {
          ...
          describe('toTx', () => {
            ...
            it('should create submittable from runtime call hex', () => {
              const tx = api.tx.system.remarkWithEvent('hello');
