 ...
 */
function converter(format: (w: string, i: number) => string): (value: string) => string {
 ...
 */
export const stringPascalCase = /*#__PURE__*/ converter(
  (w) =>
    // uppercase the first character, leave the rest unchanged
    CC_TO_UP[w.charCodeAt(0)] + w.slice(1),
);

export const stringSnakeCase = (input?: string | null) => {
  if (!input) return '';
